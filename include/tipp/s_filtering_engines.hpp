...
namespace tipp {
  namespace s {
    ...
    public:
      ...
      FIRGen_Engine(int tapsLen, IppWinType winType, IppBool doNormal) { initialise(tapsLen, winType, doNormal); }
      ...
      IppStatus highpass(Ipp64f rFreq, Ipp64f *pTaps) {
        assertIsInitialised();
        return OptionalAssertNoError(ippsFIRGenHighpass_64f(rFreq, pTaps, m_tapsLen, m_winType, m_doNormal, m_Buffer.data()));
      }

      IppStatus lowpass(Ipp64f rFreq, Ipp64f *pTaps) {
        assertIsInitialised();
        return OptionalAssertNoError(ippsFIRGenLowpass_64f(rFreq, pTaps, m_tapsLen, m_winType, m_doNormal, m_Buffer.data()));
      }

      IppStatus bandpass(Ipp64f rLowFreq, Ipp64f rHighFreq, Ipp64f *pTaps) {
        assertIsInitialised();
        return OptionalAssertNoError(ippsFIRGenBandpass_64f(rLowFreq, rHighFreq, pTaps, m_tapsLen, m_winType, m_doNormal, m_Buffer.data()));
      }

      IppStatus bandstop(Ipp64f rLowFreq, Ipp64f rHighFreq, Ipp64f *pTaps) {
        assertIsInitialised();
        return OptionalAssertNoError(ippsFIRGenBandstop_64f(rLowFreq, rHighFreq, pTaps, m_tapsLen, m_winType, m_doNormal, m_Buffer.data()));
      }
    ...
    public:
      ...
      FIRLMS_Engine(int tapsLen, float mu) { initialise(tapsLen, mu); }
      ...
      IppStatus filter(const Ipp32f *pSrc, const Ipp32f *pRef, Ipp32f *pDst, int len) { return OptionalAssertNoError(ippsFIRLMS_32f(pSrc, pRef, pDst, len, m_mu, pState)); }

      // Retrieves the tap values from the FIR LMS filter.
      void getTaps(Ipp32f *pOutTaps) { ippsFIRLMSGetTaps_32f(pState, pOutTaps); }

      IppStatus getDlyLine(Ipp32f *pDlyLine, int *pDlyLineIndex) { return OptionalAssertNoError(ippsFIRLMSGetDlyLine_32f(pState, pDlyLine, pDlyLineIndex)); }
    ...
    public:
      ...
      explicit FilterMedian_Engine(int maskSize) { initialise(maskSize); }
      ...
      void getSrcDlyLine(T *pDlyLine) { Copy(m_DlySrc.data(), pDlyLine, m_DlySrc.size()); }

      void setSrcDlyLine(const T *pDlyLine) { Copy(pDlyLine, m_DlySrc.data(), m_DlySrc.size()); }

      void getDstDlyLine(T *pDlyLine) { Copy(m_DlyDst.data(), pDlyLine, m_DlyDst.size()); }

      void setDstDlyLine(const T *pDlyLine) { Copy(pDlyLine, m_DlyDst.data(), m_DlyDst.size()); }
    };
    ...
    public:
      ...
      explicit IIR_Engine(int order, internalType *pTaps) { initialise(order); }
      ...
      void getDlyLine(internalType *pDlyLine) { IIRGetDlyLine(m_State.data(), pDlyLine); }

      void setDlyLine(const internalType *pDlyLine) { IIRSetDlyLine(m_State.data(), pDlyLine); }
    };
    ...
    public:
      ...
      AutoCorrNorm_Engine(int srcLen, int dstLen = 0, IppEnum algType = (IppEnum)(ippAlgAuto | ippsNormB)) { initialise(srcLen, dstLen, algType); }
