...
namespace tipp {
  ...
  template <class T> struct IPPallocator {
    typedef T value_type;
    IPPallocator() noexcept {} // default ctor not required by C++ Standard Library

    // A converting copy constructor:
    template <class U> IPPallocator(const IPPallocator<U> &) noexcept {}

    template <class U> bool operator==(const IPPallocator<U> &) const noexcept { return true; }

    template <class U> bool operator!=(const IPPallocator<U> &) const noexcept { return false; }

    T *allocate(const size_t n) const { return ippMalloc(n * sizeof(T)); }
    void deallocate(T *const p, size_t) const noexcept { ippsFree(p); }
  };
