...
namespace tipp {
  namespace s {
    ...
    public:
      ...
      explicit Hilbert_Engine(int length) { initialise(length); }
    ...
    public:
      ...
      IppStatus InvGetDlyLine(Ipp32f *pDlyLow, Ipp32f *pDlyHigh) { return OptionalAssertNoError(WTInvGetDlyLine<T>(m_Inv_State.data(), pDlyLow, pDlyHigh)); }

      IppStatus InvSetDlyLine(const Ipp32f *pDlyLow, const Ipp32f *pDlyHigh) { return OptionalAssertNoError(WTInvSetDlyLine<T>(m_Inv_State.data(), pDlyLow, pDlyHigh)); }

      IppStatus FwdGetDlyLine(Ipp32f *pDlyLow, Ipp32f *pDlyHigh) { return OptionalAssertNoError(WTFwdGetDlyLine<T>(m_Inv_State.data(), pDlyLow, pDlyHigh)); }

      IppStatus FwdSetDlyLine(const Ipp32f *pDlyLow, const Ipp32f *pDlyHigh) { return OptionalAssertNoError(WTFwdSetDlyLine<T>(m_Inv_State.data(), pDlyLow, pDlyHigh)); }
    };
  } // namespace s
} // namespace tipp
